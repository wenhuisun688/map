...
function section(title,items,status){
 ...
 g.append(...items.map((c,i)=>card(c,status,i<4)));sec.append(h,g);return sec;
}
...
function card(ch,status,eager){
 ...
 if(mini){
   ...
   if(ch.preview)preview.append(node('img',{src:ch.preview,alt:ch.name+' 准星预览',loading:eager?'eager':'lazy',fetchpriority:eager?'high':'low',decoding:'async',draggable:'false'}));else preview.append(node('span',{class:'crosshair-unverified',text:'待添加'}));
   el.append(preview);
   ...
   if(ch.photo){const img=node('img',{class:'player-portrait',src:ch.photo,alt:'',draggable:'false',loading:eager?'eager':'lazy',fetchpriority:eager?'high':'low',decoding:'async'});img.onerror=()=>{img.remove();el.append(node('span',{class:'player-monogram','aria-hidden':'true',text:ch.name.slice(0,2)}));};el.append(img);}
